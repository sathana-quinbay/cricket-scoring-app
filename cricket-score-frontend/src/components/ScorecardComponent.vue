  ...
  <b-card title="Card Title" no-body>
   ...
   <div class="align-Overs">
    ...
    <div v-if="items.length>2" class="fall-align">
        <b>Fall of Wickets</b>
        <div class="fall-of-wickets" v-for="(i,index) in fallOfWickets" :key=index>
            <div style="float:left;margin-left:5%; display:flex;"><div style="padding-right:60%;"><b>{{index+1}}.</b></div><b>{{i.name}}</b></div>
            <div style="float:right;margin-right:5%;">{{runs}}({{wicketBall}})</div>
            <br><hr>
            </div>
    </div>
    <div class="partnership">
         <center><b>Partnership</b></center><br>
        <div style="float:left;margin-left:5%;"><b>{{items[0].Batsman}}</b><br><span>{{items[0].R}}({{items[0].B}})</span></div>
        <b-avatar style="border:0.2px solid #319da0;font-weight:500;color:#319da0 ;" variant="light" :text="items[0].R+items[1].R"></b-avatar>
         <div style="float:right;margin-right:5%;"><b>{{items[1].Batsman}}</b><br><span>{{items[1].R}}({{items[1].B}})</span></div>
         <br><b><center style="font-size:12px;color:#319da0 ;">{{items[0].B+items[1].B}} Balls</center></b>
    </div>
  </b-card>
...
export default {
    data()
    {
       ...
       fields: ['Batsman', 'R', 'B','4s','6s','SR'],
        ...
        bowlerheadings: ['Bowler', 'O', 'M','R','W','Eco','0s','4s','6s','Wd','NB'],
        bowlers: [
          { Bowler:'Broad', O:2.1 , M:0 ,R:4,W:4,Eco:100.00,'0s':1,'4s':2,'6s':4,Wd:2,NB:3},
          { Bowler:'Bumrah' , O:2 , M:0,R:3,W:4,Eco:75.00,'0s':1,'4s':2,'6s':4,Wd:2,NB:1},
          { Bowler:'Bhuvaneshwar' , O:1, M:0 ,R:6,W:2,Eco:100.25,'0s':1,'4s':2,'6s':4,Wd:2,NB:1}
        ],
           extras:0,
           total:123,
           wickets:3,
